...
Greg is a desktop and Android utility for wrapping an arbitrary file in an authenticated,
...
A native Android client is available in [`android/`](android/). It supports Android
8.0 and newer, uses the system document picker, recognizes `.greg` files, and opens
decrypted content in an installed Android editor. It reads and writes the exact same
version-1 format as the Python desktop application.

See [`android/README.md`](android/README.md) for APK build/install instructions,
phone usage, connected-device tests, and Android-specific storage limitations.
...
Android JVM tests, APK builds, and connected-device session tests are documented in
[`android/README.md`](android/README.md). The Android tests include a fixed container
generated by the desktop implementation to enforce cross-platform compatibility.
